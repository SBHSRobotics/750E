...
void lcdDisplayShooter() {
  ...
  lcdPrint(uart1,0, speeds);
  lcdPrint(uart1,1,"<Back");
}
...
void lcdMenu() {
  while(true) {
    ...
    if(lcdReadButtons(uart1) == LCD_L) {
      option++;
      wait1Msec(100);
    } else if (lcdReadButtons(uart1) == LCD_R) {
      option--;
      wait1Msec(100);
